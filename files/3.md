...
- scp to copy my public key to the Pi ``~/.ssh/authorized_keys``
...
Looking back at my [notes](https://github.com/telliott99/Ubuntu/blob/master/ssh1.md) on ssh, the first thing I want to do is examine the configuration file for the ssh server on the Pi.
...
There's a hard way and an easy way to do this.  The easy way is to do the edit on my Mac.  I use ``scp`` to grab a copy of the file.  From the Mac
...
If you must use the Pi, it's a much better idea to use ``nano``.  ``nano`` doesn't have edit mode, or the weird key combos.  It also has nice prompts.
...
The reason for the problem was a matter of not having the target directory for the key ``~/.ssh`` and not having the right permissions and owner on the file ``authorized_keys``.  Sometimes ``root`` ends up as owner when you do ``scp``, I don't know why yet.
